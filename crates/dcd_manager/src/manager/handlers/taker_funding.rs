...
pub fn handle(
    ...
    anyhow::ensure!(fee_amount <= total_input_fee, "fee amount exceeds input value");

    let mut pst = PartiallySignedTransaction::new_v2();
    pst.global.tx_data.fallback_locktime = Some(LockTime::from_time(dcd_arguments.taker_funding_start_time)?);

    pst.add_input(Input::from_prevout(filler_token_utxo));
    pst.add_input(Input::from_prevout(collateral_token_utxo));

    let filler_to_get = collateral_amount_to_deposit / dcd_arguments.ratio_args.filler_per_principal_collateral;
