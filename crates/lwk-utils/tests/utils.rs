...
use lwk_wollet::*;
use lwk_wollet::{ElectrumClient, ElectrumUrl, WolletDescriptor};
use simplicityhl::elements::hashes::Hash;
use simplicityhl::elements::pset::PartiallySignedTransaction;
use simplicityhl::elements::{Address, AssetId, ContractHash, OutPoint, Txid};
use std::str::FromStr;
...
pub fn test_client_esplora(url: &str) -> lwk_wollet::asyncr::EsploraClient {
    let url = &url.replace("tcp://", "");
    lwk_wollet::asyncr::EsploraClient::new(ElementsNetwork::default_regtest(), &url)
}
...
/// Used with Esplora
pub async fn wait_update_with_txs(client: &mut clients::asyncr::EsploraClient, wollet: &Wollet) -> Update {
    for _ in 0..50 {
...
impl<C: BlockchainBackend> TestWollet<C> {
    ...
    pub fn fund(&mut self, env: &TestEnv, satoshi: u64, address: Option<Address>, asset: Option<AssetId>) {
    ...
    pub fn fund_explicit(&mut self, env: &TestEnv, satoshi: u64, address: Option<Address>, asset: Option<AssetId>) {
        let explicit_utxos_before = self.wollet.explicit_utxos().unwrap().len();

        let address = address.unwrap_or_else(|| self.address()).to_unconfidential();
        let txid = env.elementsd_sendtoaddress(&address, satoshi, asset);
    ...
    pub fn send_btc(&mut self, signers: &[&AnySigner], fee_rate: Option<f32>, external: Option<(Address, u64)>) {
        ...
        assert_eq!(*details.balance.balances.get(&self.policy_asset()).unwrap(), balance);
    ...
    /// Send all L-BTC
    pub fn send_all_btc(&mut self, signers: &[&AnySigner], fee_rate: Option<f32>, address: Address) {
        let balance_before = self.balance_btc();
    ...
    pub fn send_asset(
        ...
        let fee = details.balance.fee as i64;
        assert!(fee > 0);
        assert_eq!(*details.balance.balances.get(&self.policy_asset()).unwrap(), -fee);
        assert_eq!(*details.balance.balances.get(asset).unwrap(), -(satoshi as i64));
        assert_eq!(n_issuances(&details), 0);
        assert_eq!(n_reissuances(&details), 0);
    ...
    pub fn issueasset(
        ...
        let prevout = OutPoint::new(issuance_input.previous_txid, issuance_input.previous_output_index);
        assert_eq!(asset, AssetId::new_issuance(prevout, contract_hash));

        (asset, token)
    }

    pub fn reissueasset(&mut self, signers: &[&AnySigner], satoshi_asset: u64, asset: &AssetId, fee_rate: Option<f32>) {
        let issuance = self.wollet.issuance(asset).unwrap();
        ...
        let reissuance = details.issuances.iter().find(|e| e.is_reissuance()).unwrap();
        ...
        assert!(fee > 0);
        assert_eq!(*details.balance.balances.get(&self.policy_asset()).unwrap(), -fee);
        assert_eq!(*details.balance.balances.get(asset).unwrap(), satoshi_asset as i64);
    ...
    pub fn burnasset(&mut self, signers: &[&AnySigner], satoshi_asset: u64, asset: &AssetId, fee_rate: Option<f32>) {
        let balance_btc_before = self.balance_btc();
        let balance_asset_before = self.balance(asset);
        let mut pset = self
            .tx_builder()
        ...
        assert_eq!(*details.balance.balances.get(asset).unwrap_or(&0), expected_asset);
        assert_eq!(n_issuances(&details), 0);
        assert_eq!(n_reissuances(&details), 0);
    ...
    pub fn check_persistence(wollet: TestWollet<C>) {
        ...
        for _ in 0..2 {
            let wollet = Wollet::with_fs_persist(network, descriptor.parse().unwrap(), &db_root_dir).unwrap();
    ...
    pub fn make_external(&mut self, utxo: &lwk_wollet::WalletTxOut) -> lwk_wollet::ExternalUtxo {
        ...
        let tx = if self.wollet.is_segwit() { None } else { Some(tx) };
